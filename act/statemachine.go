...
func (sm *StateMachine[D]) processActions(actions []Action) {
    for _, action := range actions {
        switch action := action.(type) {
        case StateTimeout:
            sm.activeStateTimeout = ActiveStateTimeout{

                context, cancel := context.WithTimeout(context.Background(), action.Duration)
    ...
    func (sm *StateMachine[D]) invokeMessageHandler(handler any, message *gen.MailboxMessage) error {
        ...
        if !results[3].IsNil() {
            return results[3].Interface().(error)
        }

        // Check if any actions were returned
        if !isSliceNilOrEmpty(results[2]) {
        }
        ...
        // after setting up state timeouts as state timeouts are tied to the state
